...
int main(int argc, char **argv) {
  ...
  if (argc < 0) {
    nob_log(NOB_ERROR, "No input is provded\n");
  ...
  for (int i = 0; i < argc; ++i) {
      ...
      if (content.count > 0) {
        for (size_t i = 0; i < content.count-1; ++i) {
          u_int8_t x = content.items[i];
          u_int8_t y = content.items[i+1];
          map[y][x] += 1;
        }
      }
      ...
      const char *output_file_path = nob_temp_sprintf("%s.out.png", input_file_path);
      ...
      nob_log(NOB_INFO, "Generated %s", output_file_path, content.count);
